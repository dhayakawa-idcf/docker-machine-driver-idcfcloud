...
func (d *Driver) GetCreateFlags() []mcnflag.Flag {
	return []mcnflag.Flag{
		mcnflag.StringFlag{
			Name:   "idcf-api-url",
			Usage:  "IDCF Cloud API URL",
			EnvVar: "IDCF_API_URL",
		},
		mcnflag.StringFlag{
			Name:   "idcf-api-key",
			Usage:  "IDCF Cloud API key",
			EnvVar: "IDCF_API_KEY",
		},
		mcnflag.StringFlag{
			Name:   "idcf-secret-key",
			Usage:  "IDCF Cloud API secret key",
			EnvVar: "IDCF_SECRET_KEY",
		},
		mcnflag.BoolFlag{
			Name:   "idcf-http-get-only",
			Usage:  "Only use HTTP GET to execute API",
			EnvVar: "HTTP_GET_ONLY",
		},
		mcnflag.IntFlag{
			Name:   "idcf-timeout",
...
func (d *Driver) SetConfigFromFlags(flags drivers.DriverOptions) error {
	...
	if d.ApiURL == "" {
		return fmt.Errorf("idcf driver required the api-url option")
	}

	if d.ApiKey == "" {
		return fmt.Errorf("idcf driver required the api-key option")
	}

	if d.SecretKey == "" {
		return fmt.Errorf("idcf driver required the secret-key option")
	}

	if d.Template == "" {
		return fmt.Errorf("idcf driver required the template option")
	}

	if d.ServiceOffering == "" {
		return fmt.Errorf("idcf driver required the service-offering option")
	}

	if d.Zone == "" {
		return fmt.Errorf("idcf driver required the zone option")
	}
...
func (d *Driver) Create() error {
	...
	// Create the machine
	log.Info("Creating IDCF Cloud instance...")
...
func (d *Driver) Remove() error {
	...
	log.Info("Removing IDCF Cloud instance...")
